...
class CrashHandler private constructor(
    ...
    override fun uncaughtException(thread: Thread, exception: Throwable) {
        ...
        val prefs = context.getSharedPreferences("crash_handler_meta", Context.MODE_PRIVATE)
        val lastCrash = prefs.getLong("last_crash_timestamp", 0L)
        val isCrashLoop = (now - lastCrash) < 4000L
        prefs.edit().putLong("last_crash_timestamp", now).commit()

        sideEffect?.invoke(exception)

        if (isCrashLoop) {
            Auditor.critic("core", "Обнаружен бесконечный цикл падений! Отдаем управление системе.")
        ...
        try {
            ...
            val fullReport = crashReport.toText()
            val safeExtra = if (fullReport.length > 50_000) {
                fullReport.take(50_000) + "\n...[TRUNCATED: Full report saved in filesDir]"
            } else {
                fullReport
            }
            ...
            val intent = Intent(context, activity).apply {
                ...
                putExtra("CRASH_REPORT", safeExtra)
            }
    ...
    private fun cleanOldCrashReports() {
        try {
            ...
            val files = dir.listFiles { _, name -> name.startsWith("crash_report_") } ?: return
            if (files.size > 5) {
