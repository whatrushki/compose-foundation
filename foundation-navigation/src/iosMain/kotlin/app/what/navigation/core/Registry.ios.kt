...
actual fun <P : NavProvider, S : NavComponent<P>> instantiateScreen(screen: KClass<S>, provider: P): S {
    val factory = screenFactories[screen]
        ?: error("Screen factory not registered for ${screen.qualifiedName ?: screen.simpleName}. On Kotlin/Native (iOS), reflection is unsupported. Register your screen with a factory: register(${screen.simpleName}::class) { ${screen.simpleName}(it) }")
