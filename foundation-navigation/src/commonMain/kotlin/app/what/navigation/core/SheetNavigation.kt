...
class SheetNavigator(
    ...
    private val _stackEntries: MutableList<SheetProvider> = mutableListOf()
    ...
    fun <T : SheetProvider> navigateTo(
        ...
        if (launchSingleTop) {
            _stackEntries.removeAll { it::class == provider::class }
        }
...
class SheetNavGraph internal constructor(
    ...
    inline fun <reified T : SheetProvider> get(): @Composable (Any) -> Unit {
        return registry[T::class]
            ?: error("No sheet destination registered for provider ${T::class.simpleName ?: T::class}. Make sure it is registered in sheetGraph { composable<${T::class.simpleName ?: "..."}> { ... } }")
    }

    fun <T : SheetProvider> get(key: KClass<T>): @Composable (Any) -> Unit {
        return registry[key]
            ?: error("No sheet destination registered for provider ${key.simpleName ?: key}. Make sure it is registered in sheetGraph { composable<${key.simpleName ?: "..."}> { ... } }")
    }
}
